...
module Letsrate
  ...
  def unrate(user, dimension=nil)
    user.ratings_given.where(dimension: dimension, rateable_id: id, rateable_type: self.class.name).destroy_all
    true
  end
  ...
  module ClassMethods
    ...
    def letsrate_rateable(*dimensions)
      logger.debug "In letsrate_rateable!"
      ...
      has_one :rate_average_without_dimension, :as => :cacheable,
              :class_name => "RatingCache", :dependent => :destroy, :conditions => {:dimension => nil}
