...
module Letsrate
  ...
  def can_rate?(user_id, dimension=nil)
    val = self.class.connection.select_value("select count(*) as cnt from rates where rateable_id=#{self.id} and rateable_type='#{self.class.name}' and rater_id=#{user_id} and dimension='#{dimension}'").to_i
    if val == 0
      true
    else
      false
  ...
  module ClassMethods
    ...
    def letsrate_rateable(*dimensions)
      ...
      has_one :rate_average_without_dimension, -> { where dimension: nil}, :as => :cacheable, :class_name => "RatingCache",
              :dependent => :destroy
      ...
      dimensions.each do |dimension|
        ...
        has_one "#{dimension}_average".to_sym, -> {where dimension: dimension.to_s}, :as => :cacheable, :class_name => "RatingCache",
                                        :dependent => :destroy
      end
    end
  end
